...
const AssignmentTabScreen = ({ navigation }) => {
  ...
  const apiOnlineCall = () => {
    ...
    ApiMethod.gradeData(
      pass => {
        console.log('API Response received:', JSON.stringify(pass, null, 2));
        console.log('API Status:', pass.status);
        console.log('API Data:', pass.data);
  ...
  // Helper function to check if an answer is correct (copied from CompletedScreen logic)
  const isAnswerCorrect = (question) => {
    const isTruthy = (value) => {
      if (typeof value === 'boolean') return value;
      if (typeof value === 'number') return value === 1;
      if (typeof value === 'string') return value.trim().toLowerCase() === '1' || value.trim().toLowerCase() === 'true';
      return false;
    };

    if (question.hasOwnProperty('is_correct')) return isTruthy(question.is_correct);
    return false;
  };

  const isAttemptUnderReview = (item) => {
    // If questions array is available, we can accurately determine status
    if (item.questions && Array.isArray(item.questions)) {
      let textPending = 0;
      item.questions.forEach(q => {
        const type = q.question_type || q.type;
        const isCorrect = isAnswerCorrect(q);
        if ((type === 'MCQ_TEXT' || type === 'TEXT') && !isCorrect) {
          if (!item.review || item.review.trim() === "") {
            textPending++;
          }
        }
      });
      return textPending > 0;
    }

    // Fallback if questions array is missing but other signals exist
    // If percentage is 0 but user attempted it?
    // This is hard without questions. We'll trust questions array if present.
    return false;
  };
  ...
  const formatQuizData = (quizItem) => {
    ...
    const isUnderReview = isAttemptUnderReview(quizItem);
    ...
    return {
      ...
      grade: isUnderReview ? "Under Review" : ConstData.getGradeValue(quizItem.percentage),
                                ...
                                <View style={styles.footer}>
                                  <TouchableOpacity
                                    style={[
                                      ...
                                      {
                                        borderWidth: 1,
                                        borderColor: item.isUnderReview ? '#FED402' : '#34A770',
                                        backgroundColor: item.isUnderReview ? '#FFFBE6' : '#DCF3EB',
                                      },
                                    ]}>
                                    <Image
                                      source={item.isUnderReview ? require('../../../assets/images/quiz_attempt.png') : require('../../../assets/images/doneIcon.png')}
                                      style={{
                                        width: width * 6,
                                        height: height * 6,
                                        resizeMode: 'contain',
                                        tintColor: item.isUnderReview ? '#FED402' : null
                                      }}
                                    />
                                    <Text style={[styles.viewText, item.isUnderReview && { color: '#D4A017' }]}>
                                      {' '}
                                      {item.isUnderReview ? 'Under Review' : `Grade ${item.grade}(${item.grade_percent})%`}
                                    </Text>
                                  </TouchableOpacity>
                                  <TouchableOpacity
                                    onPress={() =>
