...
const CompletedScreenTablet = ({ route, navigation }) => {
  ...
  const getSelectedAnswerText = (question) => {
    console.log(`\n[CompletedScreen] Getting selected answer for question:`, {
      question_id: question.question_id,
      submitted_options: question.submitted_options,
      selected_option_id: question.selected_option_id,
      selected_options: question.selected_options,
      text_answer: question.text_answer,
      options: question.options?.map(opt => ({ id: opt.id, text: opt.option_text })),
    });
    ...
    if (textAnswer &&
      ...
      if (answerText) {
        console.log(`  ✅ Found text_answer: "${answerText}"`);
        return answerText;
      }
    }
    ...
    if (question.submitted_options !== undefined && question.submitted_options !== null) {
      ...
      console.log(`  📋 Found submitted_options:`, submittedOptionIds);
    }
    ...
    if (submittedOptionIds.length === 0) {
      ...
      console.log(`  📋 Using fallback selected_option_id/selected_options:`, submittedOptionIds);
    }

    // If no option IDs found, return early
    if (submittedOptionIds.length === 0) {
      console.log(`  ❌ No option IDs found`);
      return 'No answer selected';
    }

    // Check if we have options to match against
    if (!question.options || question.options.length === 0) {
      console.log(`  ❌ No options available to match`);
      return 'No answer selected';
    }
    ...
    const selectedOptions = question.options.filter(opt => {
      ...
      if (isMatch) {
        console.log(`  ✅ Matched option: ID ${opt.id} -> Text "${opt.option_text}"`);
      }
    ...
    if (selectedOptions.length > 0) {
      ...
      console.log(`  ✅ Final extracted answer: "${answerText}"`);
      return answerText;
    }
            ...
            <View style={styles.gradeCard}>
              ...
              <View style={styles.gradeInfoContainer}>
                ...
                <View style={styles.gradeInfoItem}>
                  ...
                  <Text style={[styles.gradeInfoValue, styles.gradeValue]}>{grade}</Text>
                </View>
              </View>
            </View>
            ...
            <View style={styles.questionsContainer}>
              {quizResult.questions && quizResult.questions.length > 0 ? (
                quizResult.questions.map((question, index) => {
                  ...
                  const userTextAnswer = getSelectedAnswerText(question);
                  const { isCorrect: isThisQuestionCorrect, isPending: isUnderReview, isTextType: isTextQuestion } = getQuestionStatus(question, quizType, overallReview);
...
export default CompletedScreenTablet;
