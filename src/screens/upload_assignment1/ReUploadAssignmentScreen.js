...
const getImagePickerOptions = (currentImageCount = 0) => ({
  title: 'Select Image',
  multiple: true,
  selectionLimit: Math.max(1, 6 - currentImageCount),
  maxWidth: 1024,
  maxHeight: 1024,
  quality: 0.8,
  mediaType: 'photo',
...
const ReUploadAssignmentScreen = ({ navigation, route }) => {
  ...
  const openCamera = async () => {
    ...
    launchCamera(currentOptions, response => {
      ...
      if (response.didCancel) {
        ...
        setSelectedImages([...selectedImages, ...source]); // Append full objects
        setPreview(true);
  ...
  const uploadDoc = (type, filePath) => {
    ...
    formData.append('assignment_id', chapterData.id);
    formData.append('syllabus_id', chapterData.syllabus_id);
...
export default ReUploadAssignmentScreen;
